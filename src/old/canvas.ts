...
export class Zovas {
    ...
    add(
        item: CanvasItem,
        x: number = (this.width - item.width) / 2,
        y: number = (this.height - item.height) / 2
    ) {
        item.move(x, y)
    ...
    getPos(x: number, y: number) {
        return [
            x - this.canvas.offsetLeft - this.prefix.x,
            y - this.canvas.offsetTop - this.prefix.y,
        ]
    }
    ...
    handleDrag(e: MouseEvent) {
        if (
            e.target ===
            this.canvas /*&& this.isInCanvas(e.clientX, e.clientY)*/
        ) {
            const [x, y] = this.getPos(e.clientX, e.clientY)
            if (e.type === 'mousedown') {
                const target = this.items
                    .filter(item => item.isHit(x, y) || item.isBorder(x, y))
                    .reverse()[0]
                if (target) {
                    if (target.isBorder(x, y)) {
                        this.resizeItem = target
                        this.resizeItem.resizeType = this.resizeItem.getHitPos(
                            x,
                            y
                        )
                        ...
                        const xb =
                            this.resizeItem.position.x +
                            this.resizeItem.width -
                            x
                        const yb =
                            this.resizeItem.position.y +
                            this.resizeItem.height -
                            y
                        this.resizeItem.resizePrefix.x =
                            Math.abs(xa) < Math.abs(xb) ? xa : xb
                        this.resizeItem.resizePrefix.y =
                            Math.abs(ya) < Math.abs(yb) ? ya : yb
                    } else {
                        target.anchor = {
    ...
    handleMove(e: MouseEvent) {
        if (e.type === 'mousemove') {
            ...
            if (this.dragItem !== null) {
                this.dragItem.move(
                    inRange(
                        x - this.dragItem.anchor.x,
                        0,
                        this.width - this.dragItem.width
                    ),
                    inRange(
                        y - this.dragItem.anchor.y,
                        0,
                        this.height - this.dragItem.height
                    )
                )
                this.render()
                ...
                if (right) {
                    this.resizeItem.width = inRange(
                        x - this.resizeItem.position.x,
                        2,
                        this.width - this.resizeItem.position.x
                    )
                }
                if (bottom) {
                    this.resizeItem.height = inRange(
                        y - this.resizeItem.position.y,
                        2,
                        this.height - this.resizeItem.position.y
                    )
                }
                if (left) {
                ...
                const mouseover = this.items
                    .filter(item => item.isBorder(x, y) || item.isHit(x, y))
                    .reverse()[0]
